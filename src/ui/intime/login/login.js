...
loginController.getTelegramCode = function(){
    ...
    // /sap/opu/odata/sap/ZTELEGRAM_AUTH_SRV/?$format=xml
    // $.ajax({

    // })
    // this.messageContainer.setMessages({severity: "warn", text: "test"})
    // document.getElementById("PASSWORD_FIELD-inner")
    // document.getElementById("USERNAME_FIELD-inner")
...
loginController.sendToken = function(){
    ...
    oHttp.open("GET", sUrl);
    oHttp.setRequestHeader("Authorization", "Basic" + btoa(this.getUserName() + ":" + this.getPassword()));
    oHttp.send();
